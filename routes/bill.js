...
router.post('/generateReport', auth.authenticateToken, (req, res) =>{
    ...
    var query = `
    insert into bill (name, uui, email, contactNumber, paymentMethod, total, productDetails, createBy)
    values (?, ?, ?, ?, ?, ?, ?, ?)`;
    ...
    connection.query(query, [id], (err, results) => {
        if (!err) {
            if (results.affectedRows > 0) {
                return res.status(200).json({ message: 'El ID de Bill no existe' });
            }
            return res.status(200).json({ message: 'Bill eliminado con éxito' });
        } else {
            return res.status(500).json(err);
        }
    });
